...
### Phase 8 — Store / Counter / Cache / SQL / MongoDB / Messaging · `app.php` tables + `src/Metrics.php` + `src/Alerts.php` + `src/Sql.php` + `src/Mongo.php` + `src/Firehose.php` + `src/Vault.php` · **Status: ✅ DONE (coroutine, v0.4.8, 2026-06-12) — Table + Redis backends both green; SQL on MariaDB 11.4.3; Mongo on rs0 replica set**
...
- **Batches:** ☑ B1 Store CRUD + typed schema + cross-worker visibility (`-w 4`: burst of hits across workers, every worker reads the same merged total) · ☑ B2 batch + iteration (mget board read; iteratePaged ring scan; set-ops viewer sets — **correctly throw on Table, work on Redis**) · ☑ B3 Counter atomicity under a 200-request burst (**exact +200, zero lost increments**) · ☑ B4 Cache getOrCompute single-flight (1st `computed=true`, 2nd `computed=false`) + TTL + tag invalidation · ☑ B5 backend matrix: metrics page green on **Table AND Redis** (`ZEALPHP_STORE_BACKEND=redis`); Tiered/CircuitBreaker/Memcached skip-recorded (no live multi-node / memcached env) · ☑ B6 **SQL**: boot migrate; incident create = incident+timeline+rule in ONE `transaction()` (1/1/1 rows); **forced-fail rollback leaves counts UNCHANGED**; joins; pool `stats()`; `mysqli()` probe → MariaDB 11.4.3 · ☑ B7 **Mongo CRUD+index**: firehose emit→find(Regex)→tag→prune round-trip; TTL `ttl_ts` + compound `route_ts` indexes created; BSON `ObjectId`/`UTCDateTime`/`Regex`/`Binary`/`Decimal128` round-trip · ☑ B8 **Mongo analytics**: aggregation rollup **matches the hand-computed control exactly**; `distinct` types; `bulkWrite` upsert (5 upserted); cursor laziness · ☑ B9 **Mongo async proof — SKIP-recorded with proof**: the eventfd non-blocking path needs the Rust `zealphp/mongodb` driver, NOT installed here; this app runs `mongodb/mongodb`+ext-mongodb (libmongoc C sockets, not coroutine-hooked → BLOCKING). Serialized timing recorded instead of a false concurrency claim · ☑ B10 **Mongo transactions + change streams** (rs0): multi-doc txn commits (a=1,b=1) / aborts (a=0,b=0) atomically; `watch()` returns an insert event + resume token · ☑ B11 **GridFS vault**: upload→download **byte-identical** (22/29 B); by-name revisions oldest-first; rename/delete; `find` listing · ☑ B12 **dual-DB contract**: §1.1 enforced in code — fixed facts in SQL, every event/rollup/vault doc in Mongo; cross-refs by id only · ☑ B13 pub/sub: alert published on one worker **handled on ALL 4 workers** (receivers=4, one delivery-log row per worker pid) · ☑ B14 reliable streams: alert audit consumed via consumer group **round-robin across workers** (distinct msg-ids, distinct consumer pids); survives restart via the group · ☑ B15 re-verify (all green).
- **Build findings (NOT new framework bugs — documented behaviour, worked around in `app.php`):** under `ZEALPHP_STORE_BACKEND=redis` a **named** `new Counter(0, 'name')` constructed in the master fatals (`predis stream_socket_client` → "API must be called in the coroutine") — this is the documented master-scope rule (phase.md Phase-8 B2.3): Atomic counters must be built pre-fork, Redis counters must bind inside a request coroutine. `app.php` now eager-builds named counters only under the Atomic backend (`$counterIsAtomic`) and the concurrency cap switches to per-key Store mode under Redis. Also corrected two ZealPulse-side slips during validation: `$request->rawContent()` is unavailable after the request ends (use buffered `php://input`); an empty `$match` in an aggregate stage throws — both fixed in this build, neither a framework defect.
- **Done:** metrics exact under burst; SQL transactions atomic with verified rollback; Mongo firehose + rollups + GridFS vault live (async honestly skip-recorded); alerts reach every worker; reliable-stream audit consumed by the group; backend switchboard green on Table AND Redis.

### Phase 9 — Lifecycle modes & isolation lab · `route/phase9.php` · **Status: ✅ DONE (coroutine-legacy, ext-zealphp 0.3.52, 2026-06-13)**
...
- **Batches:** ☑ B1 `/modes` matrix truthful in coroutine-legacy (superglobals=true, isolation=coroutine, CoSessionManager, in-process executeFile, corleg knob set + ext 0.3.52 symbols all reported live — and it correctly shows `defineIsolation:false`/`keepGlobals:false`, corroborating verification #425/#423) · ☑ B2 isolation burst: 40 concurrent × unique token → **40/40 ok, 0 leak** (g/static/superglobal all echo own token) · ☑ B3 process-state demo: 40-concurrent `/modes/tz` distinct zones → **40/40 isolated_ok** (each reads own zone after yield, no peer leak) · ☑ B4 `/modes/guard` enum rejects unknown + the 3 refused boot combos described · ☑ B5 cold-start preload burst (fresh restart, immediate 40-concurrent) → **40/40 = 200, 0 cold-autoload 500s** · ☑ B6 `/modes/ini` ini_set + putenv overlay isolation → ini_ok+env_ok · ☑ B7 re-verify (all live on ext-zealphp 0.3.52 @ HEAD f7c42f6, `PHP_INI_SCAN_DIR` pinned).
- **Done:** the lab renders the matrix live; burst probes prove zero leak; graceful degradation without ext-zealphp recorded.

### Phase 10 — CGI dispatch & the legacy bay · `route/phase10.php` + `public/legacy/` + `public/cgi-bin/` · **Status: ✅ DONE (legacy-cgi pool/proc/fork live + coroutine in-process, v0.4.8, 2026-06-13)**
- **Feature:** the **legacy bay** — a stock mod_php-era guestbook (`public/legacy/guestbook.php`, superglobals + `echo` + `session_start()`, zero framework code) running unmodified via `App::include()`; a **non-PHP shell CGI** (`public/cgi-bin/status.sh` via `cgiScriptAlias('/cgi-bin/')`) reporting host status RFC-3875-style; a return-contract probe (`legacy/contract.php`) and an env-isolation probe (`legacy/envcheck.php`); a `/phase10` strategy switchboard exercising **all four** CGI modes.
- **APIs:** `App::include('/legacy/guestbook.php')` (in-process in coroutine modes, subprocess in legacy-cgi), `App::cgiMode('pool'|'proc'|'fork'|'fcgi')`, pool tuning `cgiPoolSize()`/`cgiPoolMaxRequests()`/`cgiPoolEnvAllowlist()`/`fcgiAddress()`, `cgiScriptAlias('/cgi-bin/', ['mode'=>'proc'])` (the real signature is a `['mode'=>…]` config — the URL prefix resolves files **under the document root**, i.e. `public/cgi-bin/`, NOT a separate fs dir), RFC 3875 env (`buildCgiEnv`), sessions across the CGI boundary (#108 `mintCgiSession`), `php://input` bridge in the subprocess, `App::isolation()`/`enableCoroutine()`/`processIsolation()` getters for the switchboard.
- **Batches confirmed live:** ☑ B1 guestbook runs in pool/proc/fork (gateway `CGI/1.1`, distinct subprocess pids) + in-process under `coroutine` · ☑ B2 return contract over the boundary (`json`→200/JSON, `status`→**418**, `html`→text/html, echo+return→concat) · ☑ B3 `status.sh` via ScriptAlias — `Status: 200`→200 (parsed, not forwarded), custom `X-CGI-Kind` threaded back, `GATEWAY_INTERFACE=CGI/1.1`, `HTTP_PROXY` unset · ☑ B4 env isolation across strategies: `HTTP_PROXY` dropped in ALL (httpoxy); **pool** = `cgiPoolEnvAllowlist` honoured (`ZEALPULSE_*` visible, `SECRET_TOKEN` hidden), **proc** = `buildCgiEnv` from scratch (no app env at all — most restrictive), **fork** = full parent env leaks incl. `SECRET_TOKEN` (**live-corroborates upstream #257 residual** — `ForkPool` ignores the allowlist; tracked in zealphp-exp) · ☑ B5 session continuity across the boundary — guestbook visit# climbs 1→2→3 (pool) / 1→2 (proc) via the #108 PHPSESSID handoff · ☑ B6 pool recycle (`cgiPoolMaxRequests=2`, 20-burst) → 20/20 = 200, rotation transparent, 0 drops · ☑ B7 re-verify (fcgi-down → 502 not a hang, #289 fixed; switchboard documents the four-strategy sweep).
- **Done:** stock legacy code + a shell CGI run as first-class citizens across all four strategies; the env-exposure matrix (proc<pool<fork) recorded and corroborates the #257 fork-leak filed upstream.

### Phase 11 — Timers / Tasks / Signals / Sidecars / Outbound · `app.php` lifecycle hooks + `task/report.php` + `src/Prober.php`/`src/Background.php` + `route/phase11.php` · **Status: ✅ DONE (coroutine, v0.4.8, 2026-06-13 — Mongo firehose live)**
...
- **Batches confirmed live:** ☑ B1 `tick` fires per worker — `zp_tick_aggregations` climbs (4 workers × 5s); `clearTimer` no-throw (verified in zealphp-exp harness) · ☑ B2 after-once warmup — `zp_warmup`=4 (one-shot per worker) · ☑ B3 report task offloads to a task worker (`enqueued`, `zp_reports_built` climbs; built_by_pid ≠ request pid; aggregates Mongo `countDocuments`) — request thread never blocks · ☑ B4 prober: 4 targets probed CONCURRENTLY via `App::parallelLimit`+`HTTP::get` (each ~3–8ms in flight, wall ≈ slowest); failure isolated (bad-host → `status:0 failed:true`, no throw; self-board 404 → `ok:false failed:false` = HTTP error ≠ transport); round lands in **Mongo as one `insertMany`** (`stored_mongo:true`) · ☑ B5 `SIGUSR1` workerOnly stats-dump FIRES (`zp_statsdump` climbs when sent to HTTP workers — the #311-fixed worker path; server survives) · **caution recorded:** a master `SIGTERM`/`SIGUSR1`/`SIGUSR2` handler is shadowed by OpenSwoole's native lifecycle (boot warns `signal [15]/[10] registered by the system`) — use `SIGHUP`/custom for master, `workerOnly:true` for workers · ☑ B6 sidecar `zealphp:zp-pruner` alive in `ps`, prunes Mongo on schedule (`zp_pruner_runs` climbs), dies with the server · ☑ B7 `App::exec('uptime')` yields (returns output lines), `App::rawExec('df …')` blocks (raw string) · ☑ B8 `coproc` works in `mixed` / refused in `coroutine` (verified in zealphp-exp Phase-11 harness; ZealPulse runs coroutine) · ☑ B9 worker recycle transparent at low `ZEALPHP_MAX_REQUEST` (verified in harness — no dropped requests) · ☑ B10 re-verify (#311 worker-signal + #103 task dual-arity FIXED-confirmed upstream).
- **Cross-ref (zealphp-exp Phase 11):** the prober/`HTTP::all`/`parallel` are coroutine-mode features — under `mixed`/`legacy-cgi` they HANG the worker (filed **#429** HIGH: the `getCid()<0→Coroutine::run()` auto-wrap never completes in a reactor worker). ZealPulse runs `coroutine`, so the background machine is correct here.
- **Done:** reports build off-thread; prober rounds are concurrent + land in Mongo; the pruner sidecar runs; signals + stats live.
...
| 9 | Lifecycle modes & isolation lab | ✅ DONE (corleg, ext-zealphp 0.3.52) |
